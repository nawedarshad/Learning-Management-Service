...
$sql = "SELECT feedback_id, email, feedback FROM feedback WHERE feedback IS NOT NULL";
$result = $conn->query($sql);

// Previous database connection setup...

// Handling feedback deletion
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete']) && isset($_POST['feedback_id'])) {
...
<script>
    ...
    document.getElementById('feedback').classList.remove('link-body-emphasis');
    ...
    for (var i = 0; i < items.length; i++) {
        if (items[i].id !== 'feedback') { // Check if the id is not 'courses'
